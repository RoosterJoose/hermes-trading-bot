...
def load_strategy_snapshot(state_dir: Path, asset_key: str, version: str) -> Optional[dict]:
...
def detect_loss_clustering(trades: list, min_cluster: int = 3) -> dict:
    ...
    p = 1.0 - phi * (t * (0.319381530 + t * (-0.356563782 + t * (1.781477937 + t * (-1.821255978 + t * 1.330274429)))))
    ...
    if not n_losses:
        ...
        result["message"] = f"REGIME SHIFT: {loss_run} max consecutive losses (P={p_value:.4f}, {runs} runs vs {expected_runs:.0f} expected)"
    elif loss_run >= min_cluster:
        result["message"] = f"{loss_run} max consecutive losses (P={p_value:.4f}) — watch for further clustering"
    else:
        result["message"] = f"No significant clustering: {loss_run} max consecutive losses, {runs} runs"
...
def summarize(state_dir: Path, asset_key: str, goal: dict, trades: list) -> str:
    ...
    if current:
        ...
        lines.append(f"   Entry: {current.get('entry', {}).get('indicator', 'rsi')} < {current.get('entry', {}).get('threshold', 30)}")
        lines.append(f"   Stop Loss: {current.get('stop_loss_pct', 2.0)}%")
        lines.append(f"   Take Profit: {current.get('stop_loss_pct', 2.0) * 2}% (2x stop)")
        ...
        lines.append(f"   BTC Gate: 4h min RSI {btc.get('min_btc_4h_rsi', 25)} | 1h min RSI {btc.get('min_btc_1h_rsi', 20)}")
    ...
    if trades:
        ...
        for t in trades[-5:]:
            ...
            if btc_4h is not None:
                ...
                btc_ctx += "]"
            lines.append(f"   {t.get('exit_reason', '?')}: {t.get('pnl_pct', 0):+.2f}%{btc_ctx}")
    else:
    ...
    if score:
        ...
        lines.append(f"   Metrics: PnL={metrics.get('total_pnl_pct', 0):+.2f}% | DD={metrics.get('max_drawdown', 0)*100:.2f}% | Sharpe={metrics.get('sharpe', 0):.3f}")
    ...
    if clustering.get("max_consecutive_losses", 0) > 0:
        ...
        lines.append(f"   Max consecutive losses: {clustering['max_consecutive_losses']}")
        lines.append(f"   Runs (observed/expected): {clustering['runs']}/{clustering['expected_runs']}")
        lines.append(f"   Z-score: {clustering['z_score']:.3f} | P-value: {clustering['p_value']:.4f}")
        if clustering["regime_shift_detected"]:
            lines.append(f"   ⚠️  REGIME SHIFT detected — losses clustering beyond random chance")
    ...
    if patterns.get("findings"):
        ...
        for f in patterns["findings"]:
            ...
            lines.append(f"   {emoji} {f['condition']}: {f['count']} trades | avg {f['avg_pnl']:+.2f}% | {f['win_rate']:.0f}% WR | total {f['total_pnl']:+.2f}%")
        ...
        if len(patterns["findings"]) >= 2:
            ...
            if best["avg_pnl"] > 0 and worst["avg_pnl"] < 0:
                lines.append(f"\n💡 Key Insight:")
                lines.append(f"   Best conditions: {best['condition']} (avg {best['avg_pnl']:+.2f}%)")
                lines.append(f"   Worst conditions: {worst['condition']} (avg {worst['avg_pnl']:+.2f}%)")

    # Strategy evolution
    history_files = sorted(
        (state_dir / asset_key / "history").glob("v*.yaml")
    ) if (state_dir / asset_key / "history").exists() else []
    if history_files:
        ...
        for hf in history_files:
            ...
            lines.append(f"   {hf.name}: stop={hist.get('stop_loss_pct', '?')}% | entry={hist.get('entry', {}).get('threshold', '?')} | cooldown={cool}{btc_gate_str}{fng_str}")
    ...
    if current:
        lines.append(f"   - entry.threshold (current: {current.get('entry', {}).get('threshold', 30)})")
        lines.append(f"   - stop_loss_pct (current: {current.get('stop_loss_pct', 2.0)}%)")
        lines.append(f"   - position_size_r (current: {current.get('position_size_r', 0.5)}R)")
        lines.append(f"   - cooldown_cycles (current: {current.get('cooldown_cycles', 30)})")
        lines.append(f"   - btc_gate.min_btc_4h_rsi (current: {current.get('btc_gate', {}).get('min_btc_4h_rsi', 25)})")
        lines.append(f"   - btc_gate.min_btc_1h_rsi (current: {current.get('btc_gate', {}).get('min_btc_1h_rsi', 20)})")
        lines.append(f"   - fng_gate.min_value (current: {current.get('fng_gate', {}).get('min_value', 10)})")
        ev = current.get('evaluator', {})
        lines.append(f"   - evaluator.lower_low_cascade (current: {ev.get('lower_low_cascade', 3)})")
        lines.append(f"   - evaluator.volume_spike_mult (current: {ev.get('volume_spike_mult', 1.5)})")
        lines.append(f"   - evaluator.min_candle_position (current: {ev.get('min_candle_position', 0.3)})")
...
def has_new_trades_since_last_hypothesis(trades: list, hypotheses: list) -> tuple:
    ...
    if latest_trade > latest_hyp:
        return (True, f"new trade at {latest_trade} since last hypothesis at {latest_hyp}")
    else:
        return (False, f"no new trades since last hypothesis at {latest_hyp} (latest trade: {latest_trade})")
...
def fallback_reflect(state_dir: Path, asset_key: str, goal: dict, trades: list):
    ...
    if len(recent) < len(trades):
        print(f"   Filtered {len(trades) - len(recent)} old trade(s) — scoring {len(recent)} recent trade(s)")
    if not recent:
    ...
    if patterns.get("findings"):
        ...
        worst_pattern = sorted_findings[0] if sorted_findings[0]["avg_pnl"] < 0 else None
        best_pattern = sorted_findings[-1] if sorted_findings[-1]["avg_pnl"] > 0 else None
    ...
    if overall < 0:
        ...
        if regime_shift:
            ...
            hypothesis["reasoning"] = f"Drawdown exceeded limit. Tightened stop_loss_pct from {current_sl} to {strategy['stop_loss_pct']}"
            ...
            hypothesis["reasoning"] = f"Return under target. Loosened entry.threshold from {current_threshold} to {strategy['entry']['threshold']}"
        ...
        hypothesis["reasoning"] = f"Good performance. Widened stop_loss_pct from {current_sl} to {strategy['stop_loss_pct']} to capture more upside"
...
def main():
    ...
    parser.add_argument("--mode", choices=["fallback", "summarize", "hermes"], default="fallback")
    ...
    for asset_cfg in assets:
        ...
        if args.mode == "fallback":
            ...
            print(f"   {key}: Hermes mode runs via the cron job. Use --mode summarize to view data.")
            print("   The cron agent will read the summary, analyze it, and apply changes.")
