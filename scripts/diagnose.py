...
def build_timeofday_heatmap(trades: list) -> dict:
    ...
    """
    heat = defaultdict(lambda: {"count": 0, "total_pnl": 0.0, "avg_pnl": 0.0,
...
def build_exit_reason_analysis(trades: list) -> dict:
    ...
    by_reason = defaultdict(lambda: {"count": 0, "total_pnl": 0.0, "avg_pnl": 0.0,
                                      "wins": 0, "losses": 0, "win_rate": 0.0,
                                      "avg_rmultiple": 0.0, "total_rmultiple": 0.0})
    ...
    for k, v in by_reason.items():
        v["avg_pnl"] = round(v["total_pnl"] / v["count"], 4) if v["count"] else 0.0
        v["win_rate"] = round(v["wins"] / v["count"] * 100, 1) if v["count"] else 0.0
        v["avg_rmultiple"] = round(v["total_rmultiple"] / v["count"], 3) if v["count"] else 0.0
...
def build_per_asset_summary(trades: list) -> dict:
    ...
    by_asset = defaultdict(lambda: {"count": 0, "total_pnl": 0.0, "avg_pnl": 0.0,
                                     "wins": 0, "losses": 0, "win_rate": 0.0,
...
def format_histogram(hist: dict, trades: list = None) -> str:
    ...
    for label in buckets:
        ...
        lines.append(f"  {label:<20} {b['count']:>6} {b['pct']:>5.1f}% {sign}{b['total_pnl']:>+8.2f}%  {bar}")
    ...
    lines.append(f"\n  Symmetry check:")
    lines.append(f"    Wins (>0%): {n_wins} ({n_wins/total*100:.0f}%) | Losses (<0%): {n_losses} ({n_losses/total*100:.0f}%) | Breakeven: {n_breakeven}")
    if n_wins > n_losses:
    ...
    # Left skew detection	
    big_losses = sum(b["total_pnl"] for l, b in buckets.items() if (l.startswith("-3.") or l.startswith("-5.") or l.startswith("≤")) and b["total_pnl"] < 0)
    big_wins = sum(b["total_pnl"] for l, b in buckets.items() if (l.startswith("3.") or l.startswith("5.") or l.startswith("≥")) and b["total_pnl"] > 0)
    if abs(big_losses) > big_wins and big_losses < 0:
        lines.append(f"    ⚠️  Left-skewed: big losses (${abs(big_losses):.1f}) dwarf big wins (${big_wins:.1f})")
        lines.append(f"    → Exit logic may be cutting winners too early, letting losers run")
    elif big_wins > abs(big_losses) and big_wins > 0:
        lines.append(f"    ✅ Right-skewed: big wins (${big_wins:.1f}) > big losses (${abs(big_losses):.1f})")
...
def format_heatmap(heat: dict) -> str:
    ...
    lines.append("=" * 80)
    lines.append(f"{'Hour':<8} {'Q1(0-15m)':<14} {'Q2(15-30m)':<14} {'Q3(30-45m)':<14} {'Q4(45-60m)':<14}")
    lines.append("-" * 80)
    ...
    if enriched:
        ...
        lines.append(f"  🧊 Worst slot: {worst[0]} avg {worst[1]:+.2f}% ({worst[2]} trades)")
        if worst[1] < -0.5 and worst[2] >= 3:
            lines.append(f"  ⚠️  Structural loss detected at {worst[0]} — consider time-based trading pause")
...
def format_exit_analysis(analysis: dict) -> str:
    ...
    for reason, data in sorted_reasons:
        emoji = {"stop_loss": "🔴", "scale_out_tp1": "🟡", "chandelier_exit": "🔵"}.get(reason, "⚪")
        sign = "+" if data["avg_pnl"] >= 0 else ""
        lines.append(f"  {emoji} {reason:<20} {data['count']:>5} {sign}{data['avg_pnl']:>+8.2f}% {data['win_rate']:>4.0f}% {data['avg_rmultiple']:>+6.2f}R")
...
def format_asset_summary(summary: dict) -> str:
    ...
    lines = ["\n💼 Per-Asset Breakdown"]
    lines.append("=" * 60)
    lines.append(f"{'Asset':<12} {'Trades':>6} {'Total PnL':>10} {'Avg PnL':>9} {'WR':>5} {'Avg R':>7}")
    lines.append("-" * 60)
    
    sorted_assets = sorted(summary.items(), key=lambda x: x[1]["total_pnl"], reverse=True)
    
    for asset, data in sorted_assets:
        sign = "+" if data["total_pnl"] >= 0 else ""
        lines.append(f"  {asset:<10} {data['count']:>6} {sign}{data['total_pnl']:>+9.2f}% {sign}{data['avg_pnl']:>+8.2f}% {data['win_rate']:>4.0f}% {data['avg_r']:>+6.2f}R")
...
def main():
    ...
    parser.add_argument("--publish", action="store_true", help="Publish report to here.now")
    ...
    print(f"📈 Loaded {len(trades)} trades ({len(set(t['_asset'] for t in trades))} assets)")
    print(f"   Time range: {trades[0].get('exit_time','?')[:19]} → {trades[-1].get('exit_time','?')[:19]}")
    ...
    print(f"  Avg Win:          {sum(t['pnl_pct'] for t in wins)/len(wins):>+9.2f}%" if wins else "")
    print(f"  Avg Loss:         {sum(t['pnl_pct'] for t in losses)/len(losses):>+9.2f}%" if losses else "")
    print(f"  Profit Factor:    {abs(sum(t['pnl_pct'] for t in wins)/sum(t['pnl_pct'] for t in losses)):.2f}" if losses and sum(t['pnl_pct'] for t in losses) != 0 else "  Profit Factor:    ∞ (no losses)" if not losses else "")
    
    # Key insight
    print(f"\n💡 KEY INSIGHT:")
    if avg_r < 0:
        print(f"  The bot is losing {abs(avg_r):.3f}R per trade on average.")
        print(f"  At this rate, it will be at -2R in ~{abs(2/avg_r):.0f} more trades.")
        ...
        print(f"  Edge exists but is thin — ensure position sizing isn't over-concentrated.")
    else:
