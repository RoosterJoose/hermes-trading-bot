...
EXPECTED_YAML_SECTIONS = [
    "entry", "btc_gate", "fng_gate", "evaluator",
    "trend_filter", "kill_switches", "position_sizing",
    "chandelier_mult_major", "chandelier_mult_alts",
    "atr_sl_mult_major", "atr_sl_mult_alt",
    "atr_sl_floor_pct", "atr_sl_ceiling_pct",
    "dynamic_rsi", "hurst", "cusum",
]
...
def check_processes():
    ...
    try:
        ...
        if count == 1:
            pid = lines[0].split()[0]
            results["items"].append((PASS, f"Trading loop: PID {pid} (single instance)"))
        else:
            results["items"].append((FAIL, f"Trading loop: {count} instances — expected 1\n{out}"))
        ...
        if count2 == 1:
            ...
            results["items"].append((PASS, f"Bar ingester: PID {pid2} (single instance)"))
        else:
            results["items"].append((FAIL, f"Bar ingester: {count2} instances — expected 1\n{out2}"))
...
def check_heartbeat():
    ...
    try:
        ...
        results["items"].append((PASS, f"Positions: {len(positions)} assets ({null_count} idle)"))

        btc = hb.get("btc_context", {})
        if btc.get("btc_price"):
            results["items"].append((PASS, f"BTC ${btc['btc_price']:.0f} | 1h RSI {btc.get('btc_1h_rsi', 'N/A'):.0f}"))
        fng = hb.get("fear_greed", {})
        if fng.get("value"):
            results["items"].append((PASS, f"FnG: {fng['value']} ({fng.get('classification','')})"))
    except Exception as e:
...
def check_code():
    ...
    key_methods = [
        "_calc_adx", "_trend_filter_allows_entry", "_kill_switch_allows_entry",
        "_calc_position_size", "_calc_chandelier_exit", "_calc_atr",
        "_calc_ema_value"
    ]
    ...
    if "def filter_recent_trades" in ref_code:
        results["items"].append((PASS, f"score-pollution guard (filter_recent_trades) present"))
    else:
...
def check_log_evidence():
    ...
    try:
        ...
        result = subprocess.run(["strings", str(log_path)], capture_output=True, text=True, timeout=10)
        text = result.stdout
    ...
    for pattern in EXPECTED_GATE_LOG_PATTERNS:
        if pattern in text:
            ...
            for asset_dir in state_dir.iterdir():
                ...
                if eval_file.exists():
                    if pattern.replace(" ", "_").lower() in eval_file.read_text().lower():
                        found = True
                        break
            if found:
                results["items"].append((PASS, f"{pattern}: found in evaluator log evidence (system {int(age)}s old)"))
            else:
                results["items"].append((WARN, f"{pattern}: not yet in logs (system just restarted {int(age)}s ago)"))
        else:
        ...
        for asset_dir in state_dir.iterdir():
            ...
            if eval_file.exists():
                if "falling_knife" in eval_file.read_text():
                    ...
                    break
    if fk_found:
        results["items"].append((PASS, "Falling knife features logged in evaluator data"))
    else:
...
def check_yamls():
    ...
    for asset_dir in state_dir.iterdir():
        ...
        try:
            ...
            results["items"].append((FAIL, f"{asset_dir.name}/strategy.yaml parse error: {e}"))
            continue
        ...
        if missing:
            results["items"].append((FAIL, f"{asset_dir.name} v{ver}: missing {missing}"))
        else:
            results["items"].append((PASS, f"{asset_dir.name} v{ver}: all {len(EXPECTED_YAML_SECTIONS)} sections present"))
    return results
...
def check_bar_db():
    ...
    try:
        ...
        cur = conn.execute("SELECT asset, COUNT(*), MIN(timestamp), MAX(timestamp) FROM bars GROUP BY asset")
        rows = cur.fetchall()
        if not rows:
            ...
            for r in rows:
                ...
                results["items"].append((PASS, f"{r[0]}: {r[1]} bars ({start}→{end} UTC)"))
        conn.close()
    except Exception as e:
        ...
        if eval_file.exists():
            ...
            if lines:
                try:
                    ...
                    results["items"].append((PASS, f"{asset_dir.name}: {len(lines)} entries, last={decision}({reason})"))
                except (json.JSONDecodeError, KeyError) as e:
                    results["items"].append((WARN, f"{asset_dir.name}: {len(lines)} entries, last line parse failed: {e}"))
            else:
                results["items"].append((WARN, f"{asset_dir.name}: empty evaluator log"))
    if not found_any:
...
def check_reflection_pipeline():
    ...
    if hyp_file.exists():
        hyps = [json.loads(l) for l in hyp_file.read_text().strip().split("\n") if l.strip()]
...
def check_phase3_features():
    ...
    for asset_dir in state_dir.iterdir():
        ...
        if "dynamic_rsi:" in content:
            results["items"].append((PASS, f"{asset_dir.name}: dynamic_rsi config present"))
        else:
            results["items"].append((FAIL, f"{asset_dir.name}: dynamic_rsi config MISSING"))
    ...
    if "compute_hurst_exponent" in adaptive_code:
        results["items"].append((PASS, "compute_hurst_exponent() defined in adaptive.py"))
    else:
    ...
    # Check wiring in loop.py
    if "from hermes_trading.adaptive import compute_dynamic_rsi_threshold, compute_hurst_exponent" in loop_code:
        results["items"].append((PASS, "compute_hurst_exponent imported in loop.py"))
    else:
        results["items"].append((FAIL, "compute_hurst_exponent NOT imported in loop.py"))

    if "_compute_hurst" in loop_code:
        results["items"].append((PASS, "_compute_hurst() method defined in TradingLoop"))
    else:
        results["items"].append((FAIL, "_compute_hurst() method MISSING"))

    if "_compute_confidence_score" in loop_code:
        results["items"].append((PASS, "confidence score system integrated (replaces hurst/cusum gates)"))
    else:
    ...
    if "_compute_cusum" in loop_code:
        results["items"].append((PASS, "_compute_cusum() method defined in TradingLoop"))
    else:
        results["items"].append((FAIL, "_compute_cusum() method MISSING"))

    if "_log_skipped_setup" in loop_code:
        results["items"].append((PASS, "skipped-setup logging integrated (replaces old gate chain)"))
    else:
...
def check_crons():
    ...
    try:
        ...
        out = subprocess.run(
            ["python3", "-c", "import json; print(json.dumps(__import__('sqlite3').connect('/opt/hermes/data/cron.db').execute('SELECT name, schedule FROM jobs WHERE enabled=1').fetchall()))"],
...
def check_phase5_backtest():
    ...
    if bt_path.exists():
        ...
        for fn in ["run_backtest", "evaluate_entry", "manage_exit", "compute_metrics", "format_metrics"]:
            if fn in code:
    ...
    try:
        ...
        if metrics["total_trades"] >= 0:
            results["items"].append((PASS, f"1m backtest ran: {metrics['total_trades']} trades"))
            if metrics["total_trades"] > 0:
                results["items"].append((PASS, f"  1m: WR {metrics['win_rate']}% | Sharpe {metrics['sharpe_ratio']}"))

        # 1h test (if bars_1h table exists)
        trades_1h, metrics_1h = run_backtest("SOL_USDT", timeframe="1h")
        if metrics_1h["total_trades"] > 0:
            results["items"].append((PASS, f"1h backtest: {metrics_1h['total_trades']} trades over 30d"))
            results["items"].append((PASS, f"  1h: WR {metrics_1h['win_rate']}% | Sharpe {metrics_1h['sharpe_ratio']}"))
        elif metrics_1h["total_trades"] == 0:
            results["items"].append((WARN, "1h backtest: 0 trades (no signals in 30d 1h data)"))
        else:
...
def check_phase6_risk():
    ...
    if risk_path.exists():
        ...
        for fn in ["rolling_pearson", "compute_correlations", "correlation_allows_entry",
                    "PortfolioTracker", "compute_var", "var_position_cap"]:
            if fn in code:
                results["items"].append((PASS, f"{fn}() defined"))
            else:
                results["items"].append((FAIL, f"{fn}() MISSING"))
    ...
    for asset_key in ("SOL_USDT", "XRP_USDT"):
        ...
        if yaml_path.exists() and "var_risk_fraction" in yaml_path.read_text():
            results["items"].append((PASS, f"{asset_key}: var_risk_fraction config present"))
        else:
            results["items"].append((WARN, f"{asset_key}: var_risk_fraction config missing"))
...
def run_audit(phase=None):
    ...
    report_lines.append(f"\n{'─'*60}")
    report_lines.append(f"  Results: {PASS} {all_passed}/{total}  {FAIL} {all_failed}/{total}  {WARN} {all_warned}/{total}")
    if all_failed == 0:
        report_lines.append(f"  Verdict: PHASE {phase or '?'} PASS — ready to advance")
    else:
        report_lines.append(f"  Verdict: PHASE {phase or '?'} FAIL — address {all_failed} failure(s) before advancing")
